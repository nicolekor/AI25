...
# (추가) 테스트 완화 토글 - 기본 ON (운영 복귀 시 0으로)
TEST_RELAX = bool(int(os.getenv("TEST_RELAX", "1")))
...
# Ensemble
ENSEMBLE = dict(mode=os.getenv("ENSEMBLE_MODE", "prob"), 
                w_mn=float(os.getenv("ENSEMBLE_W_MN", "0.25")), 
                w_rn=float(os.getenv("ENSEMBLE_W_RN", "0.75")))
...
def infer_batch(model, images: List[Image.Image], T_vec=None, return_logits=True):
    x = torch.stack([tfm_eval(im) for im in images]).to(DEVICE, non_blocking=True)
    t0 = time.time()
    amp = torch.amp.autocast("cuda", dtype=torch.float16) if DEVICE=="cuda" else torch.cuda.amp.autocast(enabled=False)
    with amp:
        z = model(x)
...
def _tta2_views(img_pil: Image.Image):
    ...
    s = int(min(w,h)*0.90); left=(w-s)//2; top=(h-s)//2    # 중앙 크롭 (짧은 변의 90%)
    crops.append(img_pil.crop((left,top,left+s,top+s)))
    s2 = int(min(w,h)*0.85)           # 2) 코너 크롭(짧은변의 85%) - 좌상/우상/좌하/우하
    ...
    views = crops + [c.transpose(Image.FLIP_LEFT_RIGHT) for c in crops] # 3) 각 크롭의 좌우 플립 추가 → 5 + 5 = 10뷰
    return views

@torch.inference_mode()
def tta2_predict(mn, rn, img_pil, Tmn_vec, Trn_vec):
    views = _tta2_views(img_pil)                        # ← 여기서 10뷰 수집
    ...
    def _cm(pv):
        top2 = pv.topk(2); return float(top2.values[0]), float(top2.values[0]-top2.values[1]), 
        int(top2.indices[0])
...
def tta_quick_predict(mn, rn, img_pil, Tmn_vec, Trn_vec):
    v1 = img_pil
    v2 = img_pil.transpose(Image.FLIP_LEFT_RIGHT)       # ← 좌우 플립
...
class LeafEnsemble:
    ...
    def predict_one(self, im: Image.Image) -> Dict:
        ...
        # [합의 채택] MN/RN 동일 클래스를 가리키고, 평균확신 >= 0.8, abs(Top1-Top2) 최소마진 >= 0/25, 
        # 추가 이미지 제약(엔트로피, leaf/edge/aspect) 통과 시 -> 합의 결과 채택
        ...
        # [RNHIGH] rice가 아니고, 이미지 품질(leaf/gy/sat/edge) 기본선 통과시, RN 절대 확신(cr>= 0.25, mr>=0.3 )이면 RN 최종 선택
        ...
        decision, final_conf, reason = self._class_guard(pred_lbl, cm0, cr, mm0, mr, leaf_area, gy, sat, hi)
        ...
        if decision == "REJECT" and is_leaf and \
        ...
        # 최종
        final_lbl, final_conf = pred_lbl, final_conf
